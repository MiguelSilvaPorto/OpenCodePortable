...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const connectLocal = Effect.fn("MCP.connectLocal")(function* (
      ...
      const [cmd, ...rawArgs] = mcp.command
      const execDir = path.dirname(process.execPath)
      let portableRoot = "D:/OpenCodePortable"
      if (execDir.toLowerCase().includes("opencodeportable")) {
        portableRoot = path.dirname(execDir).replace(/\\/g, "/")
      }
      const args = rawArgs.map(arg => {
        if (typeof arg === "string" && arg.includes("D:/OpenCodePortable")) {
          return arg.replace(/D:\/OpenCodePortable/g, portableRoot)
        }
        return arg
      })
